...
const displayAllPost = (posts) => {
  ...
  posts.forEach((post) => {
    ...
    div.innerHTML = `
    <div
class="p-6 lg:p-12 flex gap-6 lg:flex-row flex-col items-center l:items-start bg-[#F3F3F5] rounded-3xl">
<div class="indicator">
...
<div
class="flex justify-between *: font-bold [&>not 
...
<button id="addToList" onclick="markAsRead('${post.description}','${
      post.view_count
    }')"
...
const markAsRead = (description, view_count) => {
  markAsReadContainer = document.getElementById("markAsReadContainer");
  const div = document.createElement("div");
  div.innerHTML = `
